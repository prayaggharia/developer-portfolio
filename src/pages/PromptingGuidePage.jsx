      ...
      <div style={{ display: "flex" }}>
        {["bad", "good"].map(t => (
          <button key={t} onClick={() => setShow(t)} style={{
            flex: 1, padding: "8px 0", border: "none", cursor: "pointer", fontFamily: "'DM Mono', monospace",
            ...
            {t === "bad" ? "✗ Bad Prompt" : "✓ Good Prompt"}
          </button>
        ))}
      </div>
      <div style={{ padding: 16, background: show === "bad" ? "#1a0f0f" : "#0f1a12", minHeight: 80 }}>
        <p style={{ margin: 0, fontFamily: "'DM Mono', monospace", fontSize: 12, color: show === "bad" ? "#ff9999" : "#a0f0a0", lineHeight: 1.6 }}>
          "{show === "bad" ? bad : good}"
        </p>
      </div>
      {show === "good" && (
        <div style={{ padding: "10px 16px", background: "#0d1a10", borderTop: "1px solid #1e3a24" }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#7a8a7c" }}>{why}</span>
        </div>
      )}
    </div>
  );
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .pg-back-nav { position: fixed; top: 64px; left: 0; right: 0; z-index: 50; padding: 10px 40px; background: #0a0a0a; border-bottom: 1px solid #1a1a1a; display: flex; align-items: center; }
      ...
      {/* Header */}
      <div style={{ padding: "48px 40px 32px", borderBottom: "1px solid #1a1a1a", position: "relative", overflow: "hidden" }}>
        ...
        <p style={{ margin: "0 0 8px", fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: 3, color: "#555", textTransform: "uppercase" }}>Claude × Anthropic</p>
        <h1 style={{ margin: 0, fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: 42, color: "#f0f0f0", lineHeight: 1.1 }}>
          Prompting<br /><span style={{ color: "#F4A261" }}>Guide</span>
        </h1>
        <p style={{ marginTop: 12, color: "#666", fontSize: 14, maxWidth: 480 }}>A visual, step-by-step breakdown of every technique — from general best practices to task-specific strategies.</p>
      </div>

      {/* Section Tabs */}
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #1a1a1a", overflowX: "auto" }}>
        {data.sections.map((s, i) => (
          <button key={s.id} onClick={() => { setActiveSection(i); setActiveTip(0); setActiveCat(0); }} style={{
            padding: "16px 28px", border: "none", cursor: "pointer", background: activeSection === i ? "#141414" : "transparent",
            ...
            whiteSpace: "nowrap", transition: "all 0.2s", display: "flex", alignItems: "center", gap: 8
          }}>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 260px)" }}>
        ...
        {activeSection === 0 && (
          <>
            ...
            <div className="fade" key={activeTip} style={{ flex: 1, padding: "36px 40px", overflowY: "auto" }}>
              {(() => {
                const tip = section.tips[activeTip];
                return (
                  <>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 24 }}>
                      <div style={{ width: 56, height: 56, borderRadius: 14, background: "#141414", border: `1px solid ${section.color}33`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>
                        {tip.icon}
                      </div>
                      <div>
                        ...
                        <h2 style={{ margin: 0, fontFamily: "'Playfair Display', serif", fontSize: 26, color: "#f0f0f0" }}>{tip.title}</h2>
                      </div>
                    </div>

                    <div style={{ padding: "16px 20px", background: "#111", borderRadius: 10, borderLeft: `3px solid ${section.color}`, marginBottom: 24 }}>
                      <p style={{ margin: 0, fontSize: 14, color: "#c0c0c0", lineHeight: 1.7 }}>{tip.rule}</p>
                    </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 32 }}>
                      {activeTip > 0 ? (
                        <button onClick={() => setActiveTip(activeTip - 1)} style={{ padding: "10px 20px", background: "#141414", border: "1px solid #2a2a2a", borderRadius: 8, color: "#888", cursor: "pointer", fontFamily: "'DM Mono', monospace", fontSize: 11 }}>
                          ← Prev
                        </button>
                      ) : <div />}
                      {activeTip < section.tips.length - 1 && (
                        <button onClick={() => setActiveTip(activeTip + 1)} style={{ padding: "10px 20px", background: section.color, border: "none", borderRadius: 8, color: "#000", cursor: "pointer", fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 600 }}>
                          Next →
                        </button>
                      )}
                    </div>
        ...
        {activeSection === 1 && (
          <div style={{ flex: 1, padding: "36px 40px", overflowY: "auto" }}>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 32 }}>
              {section.categories.map((cat, i) => (
                <button key={i} className="cat-pill" onClick={() => setActiveCat(i)} style={{
                  padding: "10px 20px", borderRadius: 100, border: `1px solid ${activeCat === i ? section.color : "#2a2a2a"}`,
                  background: activeCat === i ? `${section.color}22` : "#111",
                  color: activeCat === i ? section.color : "#666", cursor: "pointer",
                  fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: 1, transition: "all 0.2s",
            ...
            <div className="fade" key={activeCat}>
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, color: "#f0f0f0", marginBottom: 8 }}>
                {section.categories[activeCat].icon} {section.categories[activeCat].name}
              </h2>
              <p style={{ color: "#555", fontSize: 13, marginBottom: 28 }}>Follow these techniques for best results</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
                {section.categories[activeCat].tips.map((tip, i) => (
                  <div key={i} style={{ padding: 24, background: "#111", borderRadius: 14, border: "1px solid #1e1e1e", position: "relative", overflow: "hidden" }}>
                    <div style={{ position: "absolute", top: 16, right: 16, fontFamily: "'DM Mono', monospace", fontSize: 32, color: "#1a1a1a", fontWeight: 900 }}>{i + 1}</div>
                    <div style={{ width: 36, height: 3, background: section.color, borderRadius: 2, marginBottom: 14 }} />
                    <h3 style={{ margin: "0 0 10px", fontSize: 15, color: "#e0e0e0", fontWeight: 600 }}>{tip.title}</h3>
                    <p style={{ margin: 0, fontSize: 13, color: "#777", lineHeight: 1.65 }}>{tip.desc}</p>
                  </div>
                ))}
              </div>

              <div style={{ marginTop: 32, padding: 24, background: "#0d1a14", borderRadius: 14, border: `1px solid ${section.color}33` }}>
        ...
        {activeSection === 2 && (
          <div style={{ flex: 1, padding: "48px 40px", overflowY: "auto" }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, color: "#f0f0f0", marginBottom: 8 }}>Troubleshooting & Best Practices</h2>
            <p style={{ color: "#555", fontSize: 14, marginBottom: 40 }}>Reduce hallucinations and maximize Claude's performance.</p>

            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {section.tips.map((tip, i) => (
                <div key={i} className="fade" style={{
                  display: "flex", gap: 20, padding: 28, background: "#111", borderRadius: 16,
                  border: "1px solid #1e1e1e", alignItems: "flex-start",
                  animationDelay: `${i * 0.1}s`
                }}>
                  <div style={{
                    width: 52, height: 52, borderRadius: 14, background: `${section.color}22`,
                  ...
                  <div style={{ flex: 1 }}>
                    <h3 style={{ margin: "0 0 8px", fontSize: 17, color: section.color, fontWeight: 600 }}>{tip.title}</h3>
            ...
            <div style={{ marginTop: 36, padding: 28, background: "#0f0f1a", borderRadius: 16, border: `1px solid ${section.color}33` }}>
              <p style={{ margin: "0 0 14px", fontFamily: "'DM Mono', monospace", fontSize: 11, color: section.color, letterSpacing: 2, textTransform: "uppercase" }}>⬡ Golden Rules at a Glance</p>
              {[
                ...
                <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 13, color: "#999", lineHeight: 1.5 }}>{rule}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Footer Progress */}
      <div style={{ borderTop: "1px solid #1a1a1a", padding: "16px 40px", display: "flex", alignItems: "center", gap: 16 }}>
